...
function MyPets() {
  const { fetchSavedPets, fetchAdoptedPets, fetchFosteredPets } =
    useContext(UsersContextInstance);
  const { loggedInUserID } = useContext(AuthContextInstance);
  const { fetchInfo, isLoading, setIsLoading } =
    useContext(UsersContextInstance);
      ...
      <Stack width={["100%", "80%", "65%", "55%"]}>
        ...
        >
          <TabList
            ...
            mb="1em"
          >
            <Tab fontSize={["md", "lg", "xl", "2xl"]}>Saved Pets</Tab>
            <Tab fontSize={["md", "lg", "xl", "2xl"]}>Fostered Pets</Tab>
            <Tab fontSize={["md", "lg", "xl", "2xl"]}>Adopted Pets</Tab>
          </TabList>
          <TabPanels>
            <TabPanel>
