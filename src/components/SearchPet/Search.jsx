...
function Search({ searchInput }) {
  ...
  const { fetchInfo, setErrorMsgClient } = useContext(UsersContextInstance);
  ...
  async function getPetbyType(type) {
    try {
      setLoading(true);
      const res = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/pets/search/${type}`,
  ...
  async function handleSubmit(e) {
    try {
      ...
      setLoading(true);
      const res = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/search?type=${type}&name=${name}&status=${status}&height=${height}&weight=${weight}`,
        { withCredentials: true },
      );
  ...
  return (
    <div className="main-container">
      ...
      <Stack spacing={4} width={["90%", "80%", "65%", "60%"]}>
        ...
        <Stack direction="row">
          <FormControl
            onChange={(e) => setAdvSearch(!advSearch)}
            display="flex"
            alignItems="center"
          >
        ...
        {numResultsS && (
          <Stack direction="row">
            ...
            <Spacer />
            <Button w="11em" size="sm" colorScheme="gray" onClick={clearSearch}>
              Clear search
            </Button>
          </Stack>
        )}
