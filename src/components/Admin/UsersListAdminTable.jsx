...
function UsersListAdminTable() {
  ...
  const fetchResults = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/usersadmin?page=${page}&limit=5`,
      );
          ...
          <>
            ...
            <Flex mt={3}>
              <Button
                ...
                disabled={page === 1}
              >
                <Hide below="sm">Previous Page</Hide>
              </Button>
              <Spacer />
              <Text className="main-font">
                {" "}
                Displaying {[page - 1] * 5 + countResults} out of {totalResults}{" "}
                users
              </Text>
