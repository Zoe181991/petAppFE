  ...
  const deletePet = async () => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_SERVER_URL}/admin/deletepet/${pet._id}`,
      ...
      >
        ...
        >
          ...
          <Stack fontSize={["sm", "md", "md"]}>
            <Text>
              <span className="bold">Name:</span> {pet?.name}
            </Text>
            <Text>
              <span className="bold">Type:</span> {pet?.type}
            </Text>
            <Text>
              <span className="bold">Breed:</span> {pet?.breed}
            </Text>
          </Stack>
          <Spacer />
          <Stack>
          ...
          <Skeleton isLoaded={!isLoading}>
            <FormControl mt={4}>
              <InputGroup className="main-font">
                <Checkbox
                  isChecked={hypoallergnic}
                  value={hypoallergnic}
                  onChange={(e) => setHypoallergnic(!hypoallergnic)}
                  size="md"
                  colorScheme="purple"
                >
